...
统计总数
		...
		> db.myCol.count()
		100
		> db.myCol.count({"x" : {"$gt" : 70}})
		30
		> 

### distinct
用于查询出在该集合中，某个键的所有不同的值。runCommand。要指定集合和key

		> db.runCommand({"distinct":"myCol" , "key" : "y"})
		{
			"values" : [
				0,
				1,
...
### group
根据键的值分组，很像关系型数据库的group by。  
先构造一个数据，y的值变为x模10的值。

		> for(i=1; i<=100; i++){ db.myCol.update({"x":i},{"$set" : {"y" : NumberInt(i/10)}}); }
		WriteResult({ "nMatched" : 1, "nUpserted" : 0, "nModified" : 0 })
		> db.myCol.find()
		{ "_id" : ObjectId("5615dd40ea202bacf71960db"), "x" : 1, "y" : 0 }
		{ "_id" : ObjectId("5615dd40ea202bacf71960dc"), "x" : 2, "y" : 0 }
		{ "_id" : ObjectId("5615dd40ea202bacf71960dd"), "x" : 3, "y" : 0 }
		{ "_id" : ObjectId("5615dd40ea202bacf71960de"), "x" : 4, "y" : 0 }
		{ "_id" : ObjectId("5615dd40ea202bacf71960df"), "x" : 5, "y" : 0 }
		{ "_id" : ObjectId("5615dd40ea202bacf71960e0"), "x" : 6, "y" : 0 }
		{ "_id" : ObjectId("5615dd40ea202bacf71960e1"), "x" : 7, "y" : 0 }
		{ "_id" : ObjectId("5615dd40ea202bacf71960e2"), "x" : 8, "y" : 0 }
		{ "_id" : ObjectId("5615dd40ea202bacf71960e3"), "x" : 9, "y" : 0 }
		{ "_id" : ObjectId("5615dd40ea202bacf71960e4"), "x" : 10, "y" : 1 }
		。。。

根据y进行分组，cond可以加个条件也可不加，reduce进行处理，initial设定一个初始值。

		> db.myCol.group({ 
			key: {y:1}, 
			cond : {y : {"$lt" : 6}}, 
			reduce : function(cur, result){result.x += cur.x}, 
			initial:{x : 0} 
			})
		[
			{
...
### MapReduce
可以执行非常强大的聚合功能。厉害的地方在于它是一个可以轻松并行化到多个服务器的聚合方法。它会拆分问题，再将各个部分发送到不同的机器上，让每台机器都完成一部分，最后再把结果汇集起来。  
MapReduce的功能强大，但是牺牲了速度，决不能用在实时环境中。正确的用法是所谓后台任务运行MapReduce,MapReduce会创建一个保存结果的集合，可以对这个集合进行实时查询。   
借用一下mongo官网的图 ![](https://docs.mongodb.org/manual/_images/map-reduce.png)
map函数使用emit返回要处理的值。  
